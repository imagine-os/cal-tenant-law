...
Shells: `customer` -> PhoneShell; `public` -> bare + `SiteLayout` (P-01..P-04 and the public stubs); `dev`, `docs`, `opposition` -> DesktopShell with their own menus (the legal pages K-10..K-13 ride the `docs` surface); every other staff surface -> the one staff DesktopShell filtered per role.
...
### 1.4 Tables (24 app tables + the `user_roles` RLS helper; `supabase/schema.sql` and `docs/data-model.md` are generated by `npm run sql`)
