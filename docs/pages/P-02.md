...
| `site.openPlan` | open the live project plan | none | — | yes |
...
None. Every control on this page does what it says; the links to `/#/plan` and `/#/board` open real pages (the project board asks for a staff role, which is expected from the public site).

## Real vs mock

Real: the counts, every pass and status, and the task lists in the drawer — all read from the plan file the builders work from. The feature descriptions are written copy, not data from the system; Pass 5 (T-112) replaces them with real screenshots and verified prices and vendors.
