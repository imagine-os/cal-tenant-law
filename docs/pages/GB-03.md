...
| --- | --- | --- |
| `board_node_meta` | read | One row per square, seeded with `typical_cost_band: null`, `deadline_rule: null`, `cost_source: null`, `deadline_authority: null` and a note saying why. The explanation section reads the rows and states how many are filled (0 of 88) |
...
Every cost and deadline chip and every overlay badge. Counted by `data-placeholder`; each names its task (T-074, T-059) in dev mode.

## Real vs mock

Real: the overlay mechanism, the `board_node_meta` table and its rows, the scenario tree, the two-level "and from there" reading. Mock: nothing is mocked — the numbers are absent, which is the point. T-076 turns this page into the real cost overlay once the cost model and deadline engine exist, and adds the 3D object view (GB-04) on the same data.

## Inputs and responsive check (P-01, P-03)

Checked at 360, 390, 768, 1280, 1920, 2560, 3840 in light and dark (`npm run qa:responsive -- --codes=GB-03`: 14 cells, 0 failing, 0 a11y findings). Board and panel sit side by side from 1200 px and stack below it; the panel scrolls independently on wide screens. The narrower board container drops to a two-square layout band so labels stay legible next to the panel.
