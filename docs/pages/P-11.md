...
9. **Sources** — the evidence flag, what it means, and the store URLs the facts were indexed from.
...
- `RULE-CATALOG-01` — the price carries its note and the unverified badge; "price not listed" when nothing was indexed.
...
- The evidence line distinguishes `verified-snippet` (quoted from an indexed listing) from `inferred` (reconstructed from URLs, titles and partial snippets).
