...
- The link to the live project board renders only for super admin and owner, because `/plan/*` is staff-only (D-048); every other reader stays on this page, which already carries the same numbers.
...
Everything numeric on this page is real: it is the plan file, read at build time (136 tasks, including Pass 2 wave A's T-118..T-136). Nothing is written. When a task's status changes in `tasks.json`, this page changes with it.
...
- `docs/changelog/_pending/site-people.md` (0.2.0-dev: the project-board link renders only for the roles `/plan` allows)
