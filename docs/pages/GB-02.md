...
`PageHeader`, `GameBoard`, `BoardKey`, `Drawer`, `SegmentedControl` (up to five cases) / `Select` (above five), `Card`, `Section`, `EmptyState`, `Badge`, `StatusBadge` vocabulary via `Badge`, `Button`, `Placeholder`, `Toast`.
...
Real: positions, move history, the move write, the derived visited path, case switching, case mode rendering. Mock: the demo cases and their people are fictional and seeded (`src/data/seed/board.ts`); the selector lists whatever `board_positions` rows exist, so it follows the seed; real positions arrive with the cases module (T-055), which will link the case detail page (L-10) to this route. A move made in the demo survives a reseed (the mock provider keeps rows it did not seed).

## Inputs and responsive check (P-01, P-03)

Checked at 360, 390, 768, 1280, 1920, 2560, 3840 in light and dark (`npm run qa:responsive -- --codes=GB-02`: 28 cells across both routes, 0 failing, 0 a11y findings). Re-checked 2026-09-20 with Playwright at 360 / 768 / 1280 / 1920: `/board/case` redirects to the first case with a position, the selector lists every case that has one, the current square carries "YOU ARE HERE", the visited path is drawn and the next moves are listed with their path type; an unknown case id shows its own `EmptyState`; 0 console errors. Fixed in the same pass: the selector overflowed the page header once the seed grew past a handful of cases, and an unknown case id used the "no demo cases" wording. Case selector, move buttons and board squares are all keyboard reachable in a sensible order; the current square is panned into view on load; the "You are here" and "What happens next" cards stack on one column under 900 px.
