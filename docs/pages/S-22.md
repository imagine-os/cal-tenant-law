...
**FeedbackButton (0.2.0 release fix)**: at 1920 / 2560 / 3840 the shell's floating Feedback button sat on the sticky research pane (the 1024 px `.mode-document` rule outranked the wider rules). `drafting.css` now sets `--fab-right` on the shell (pane column + shell padding + gap) and `FeedbackButton.css` reads it, so the button lands over the sheet's corner at every width and stays reachable; measured clear of the pane at 1600 / 1920 / 2560 / 3840, `qa:responsive --codes=S-22` 14 cells ok.
...
- `docs/changelog/0029-release-0.2.0.md` (0.2.0: FeedbackButton offset through `--fab-right`)
