...
Enriched by T-024: every surface card now carries a **live preview of its home page running as that family's demo role**, an explicit "Enter as <demo person>" button and a built / stub / planned badge from the route manifest; a **Testing hub** row opens the canvas (D-21), the simulator (D-22), the plan, the board, the proposal, the docs, the manual, the legal memory and the builder tools.
...
1. HubHeader - brand mark, version badge, LangToggle, brand cycle, theme toggle, builder-tool Toggle (super admin only)
2. Hero - eyebrow, title, subtitle, RoleSwitcher (demo user + view as); dev-mode hint with Kbd Ctrl + .
3. SurfaceGrid - one Card per family (P, C, GB, F, L, S, O, A, X, MK, PM, M, K, D): icon, name, code range, built/stub/planned badge, description, a live preview of the family home as its demo role, and "Enter as <demo name>" with the role and path; a dot marks the family of the current role
4. TestingHub - Canvas, Demo simulator, Plan, Game board, Proposal, Docs, Ops manual, Legal memory, Dev tools, each with its status; a path with no route yet is a Placeholder, never a dead link
5. Footer - version and counts: routes / built / stubs, tables, rules, components, actions, plan tasks done / total; mock-data note
...
Card, Button, Toggle, Badge, StatusBadge, Icon, IconButton, Kbd, Placeholder, RoleSwitcher, LangToggle, PhoneFrame, DeviceFrame

## Placeholders

Testing-hub cards whose route does not exist yet (Proposal `/site/proposal`, Legal memory `/legal`) are wrapped in `Placeholder` with the module that will build them. Everything else is live; every stub the cards lead to shows its own planned actions as Placeholders.
...
- `docs/changelog/_pending/foundation.md` (base page)
- `docs/changelog/_pending/showcase.md` (T-024 enrichment)
