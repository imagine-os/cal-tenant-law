...
Where the firm corrects its own menu. Every price in CTL OS today is "as listed on caltenantlaw.com on 2026-09-18" (the live scrape, D-040) and not yet confirmed by the firm, so the single most valuable thing the owner can do in this system is sit with this table and say which figures are right. Each row edits in place through the provider, the verified toggle removes the unverified badge everywhere that price appears, and "reset from repo JSON" puts the repository file back in charge.
