...
| `services` | read | 98 rows seeded from `docs/data/services-catalog.json` (live scrape 2026-09-18, D-040); only `active` rows render, in `store_order` inside each category |
| `illustrations` | read | the firm's product circle icons and category tiles (`FirmIcon`, category headers; D-042) |

## Rules

- `RULE-CATALOG-01` — every price renders with the "as listed on caltenantlaw.com on 2026-09-18 · unverified" badge and its tooltip until `services.verified` is set; a service with no indexed price says "price not listed", never `$0`.
...
The catalog is the live scrape of caltenantlaw.com taken on 2026-09-18 (`docs/data/services-catalog.json`, 20 menu categories + 28 hidden stage categories / 98 services / 98 priced or free, D-040); every price badge reads "as listed on caltenantlaw.com on 2026-09-18 · unverified"; the live site was unreachable from the build environment, so nothing was read from a page and every row is `verified: false`. Board squares and phases are the real poster data. Buying is not wired. When Supabase lands the rows move behind the same provider interface and the JSON becomes the migration seed.
...
- `docs/changelog/0012-services-catalog.md` (0.1.1)
