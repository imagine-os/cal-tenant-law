...
| `screenshots/<CODE>/<width>[-dark].jpg` | Captures from `npm run screenshots` (390 + 1280, dark and 3840 for key pages), plus `routes.json`. |
...
| `game-board/` | `README.md` (the board transcribed and explained) and `nodes.json` (phases, nodes, edges, key). Source of truth for GB-xx pages; cost and deadline fields are filled in Pass 2 from `legal/`. |
...
| `data/` | Machine-readable facts about the firm scraped live from caltenantlaw.com on 2026-09-18 (prompt 0004, D-038): `services-catalog.json` (the store: 98 services, 48 categories incl. the hidden stage tree; seeds `service_categories` / `services`), `videos.json` (seeds `lessons`), `offices.json` (seeds the office `tenants`), `illustrations.json` (seeds `illustrations`; files in `../reference/site-scrape/assets/`), `articles.json`, `faq.json`. Every row carries `evidence: scraped-live`, `scraped_at`, `verified: false`; correcting a JSON corrects the product (RULE-CATALOG-04). |
| `reference/` | Digests of what we studied and adopt: `hoy-petrock-patterns.md`, `graph-gallery-views.md`, `company-os.md` (seam only), `firm-site-digest.md`; generated by the code workers: `surfaces.md` (routes, provider methods, scripts, actions, MCP / CLI / API), `annotations-triage.md`. |
...
| `qa/` | Generated QA reports (responsive matrix `responsive-report.{md,json}`, bundle budget, a11y). Release artifacts: committed at each release. |
