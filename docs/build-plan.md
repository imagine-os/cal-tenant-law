...
| 1 | Foundation, first modules, release 0.1.0 | 52 _(was 46; T-114..T-117 and the 0.1.1 / 0.1.2 tasks were added after the table)_ | 23 _(was 18)_ | 27 _(was 26)_ | 2 | Pass 0 done. Modules start when the foundation gate (build green, HUB-01 renders every role, contract in CLAUDE.md) is true. |
| 2 | Core operations, release 0.2.0 (wave A) and 0.2.1 (wave B) | 52 _(was 35; +T-118..T-136 on 2026-09-20, prompt 0006)_ | 14 _(was 9)_ | 34 _(was 24)_ | 4 _(was 2)_ | Release 0.1.0 shipped; case domain schema (T-054) merged before the pages that read it. Wave A (D-045) gates on the order pipeline domain (T-120) and DocPreview (T-121) for the pages that read them. |
...
| | **Total** | **136** _(was 113)_ | **57** _(was 47)_ | **70** _(was 59)_ | **9** _(was 7)_ | |
...
### Wave A (prompt 0006, 2026-09-20)

Justin's 2026-09-20 asks (prompt 0006) reorder Pass 2 without renumbering it (D-045): **wave A** is what he asked for now and ships as **release 0.2.0** (T-088, retargeted to T-135); the rest of Pass 2 is **wave B** and ships as **0.2.1** (T-136, which took over T-088's old dependencies T-086 / T-087). Existing tasks pulled into wave A keep their ids and are marked `doing` with a wave note in `tasks.json`; T-054 is scoped this pass to unifying the demo case seeds (one case id set, board positions and move histories for every ops case) plus the order pipeline (T-120), with the full case lifecycle staying in wave B. Parallel group A = every wave A module task (disjoint folders, one Opus 5 worker each); Fable does the domain (T-120), DocPreview (T-121), role scoping (T-119) and integration (T-134); Sonnet 5 does portraits (T-133) and QA / Spanish (T-135).

| Id | Code | Task | Lane | Depends on | Model | Status | Size | Tick | Group |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-118 | DOC | Prompt 0006 and pass-two plan: prompt log, wave A / wave B split, tasks T-118 onward, decisions D-045..D-051, kanban, build plan | QA & Docs | T-117 | fable | done | S | 16 | A |
| T-119 | CORE | Role scoping audit: route roles as the source of truth, shell menus, hub cards, role matrix page D-24 | Foundation | T-117 | fable | doing | M | 16 | A |
| T-120 | CORE | Order pipeline domain: stages, waiting-on, schema (orders, order_stage_events, client_requests, calls, follow_ups), seeds | Legal team | T-054 | fable | doing | L | 12 | A |
| T-121 | D-02 | DocPreview organism: preview image of any document kind (pleading, letter, court form, evidence photo, email, text thread, receipt, video) | Documents | T-117 | fable | doing | M | 16 | A |
| T-122 | D-21 | Canvas rebuild: zoomable canvas of browser windows, drag and resize per window, saved layouts in page_layouts, flows layer of what each role can do | Hub & Dev tools | T-119, T-121 | opus-5 | doing | L | 17 | A |
| T-123 | HUB-01 | Homepage order by owner priority; role cards show only that role's surfaces | Hub & Dev tools | T-119 | opus-5 | doing | S | 17 | A |
| T-124 | D-22 | Demo simulator: each demo opens in the device it represents (client app phone, staff desktop, owner TV), device chrome per preset | Hub & Dev tools | T-117 | opus-5 | doing | M | 16 | A |
| T-125 | L-13 | Attorney document pipeline board (L-13) and order detail (L-14), paralegal queue (S-13) | Legal team | T-120, T-121 | opus-5 | doing | L | 17 | A |
| T-126 | C-11 | Client "my orders": client-facing stage labels, what we need from you, approve or request changes | Client | T-120 | opus-5 | doing | M | 13 | A |
| T-127 | F-14 | Front desk order status lookup: any order by client name, phone or order id | Front desk | T-120 | opus-5 | doing | S | 13 | A |
| T-128 | F-12 | Front desk call console (F-12: incoming calls, caller match, orders in hand, notes, outcome), follow-ups board (F-15: due dates, items needed from clients, calls to return), clients directory (F-13) | Front desk | T-120 | opus-5 | doing | L | 13 | A |
| T-129 | S-21 | Drafting studio: pleading-paper editor with side panel (relevant laws, precedent, recommendations, client details), template catalog by board node, client questions and item requests sent to the client | Documents | T-120, T-121, T-069 | opus-5 | doing | XL | 17 | A |
| T-130 | C-40 | LMS: courses from videos.json and articles, in-app player with watched state, journey by board position (C-41), staff insight (L-40), LMS admin (A-11) | Learning | T-077 | opus-5 | doing | L | 12 | A |
| T-131 | C-20 | Client binder and evidence: upload from phone or desktop (C-20), requests checklist (C-21), connect email / text / WhatsApp seam with mock import (C-22), staff binder (L-31) | Discovery | T-120, T-121 | opus-5 | doing | L | 17 | A |
| T-132 | GB | Game board verification against the poster PDF: labels, edges, phases, routes, keyboard, fixes | Game board | T-037 | opus-5 | doing | M | 7 | A |
...
| Id | Code | Task | Lane | Depends on | Model | Status | Size | Tick | Group |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-054 | CORE | Case domain: schema and lifecycle (cases, matters, parties, people, roles-on-case, events, board positions, assignments) _(status / dependencies changed 2026-09-20)_ | Legal team | T-053 | fable | doing | L | 11 | A |
| T-063 | F-10 | Front desk intake queue: initial and follow-up consultation forms as records, triage, convert to case _(status / dependencies changed 2026-09-20)_ | Front desk | T-054 | opus-5 | doing | M | 12 | A |
| T-065 | F-12 | Call log and client directory (F-12 calls with hotline minutes, F-13 clients) _(status / dependencies changed 2026-09-20)_ | Front desk | T-054 | opus-5 | doing | S | 12 | A |
| T-069 | CORE | Pleading-paper editor spike: browser rich-text with numbered-line ruler, caption block, footer, PDF and DOCX export _(status / dependencies changed 2026-09-20)_ | Documents | T-053 | fable | doing | L | 11 | A |
| T-070 | C-20 | Client binder: every document, evidence item and court paper organised by board stage, with status and what is missing _(status / dependencies changed 2026-09-20)_ | Discovery | T-054 | opus-5 | doing | L | 12 | A |
| T-071 | C-21 | Discovery requests to the client: staff ask for documents, client uploads from phone, reminders, checklist by request _(status / dependencies changed 2026-09-20)_ | Discovery | T-070 | opus-5 | doing | M | 13 | A |
| T-072 | CORE | Evidence import seams: email and text-message ingestion (forward-to address, screenshot / export parsing), tagging, chain of custody fields _(status / dependencies changed 2026-09-20)_ | Discovery | T-070 | fable | doing | M | 13 | A |
| T-078 | C-40 | Learning: C-40 home (what to watch next by board position), player with watched state, C-41 journey drip rules, L-40 "what my client has watched" _(status / dependencies changed 2026-09-20)_ | Learning | T-077, T-054 | opus-5 | doing | L | 12 | A |
| T-088 | CORE | Release 0.2.0: Pass 2 wave A _(status / dependencies changed 2026-09-20)_ | QA & Docs | T-135 | fable | todo | S | 20 | A |

Every other Pass 2 task (T-055..T-062, T-064, T-066..T-068, T-073..T-076, T-080..T-087) is wave B (`notes: "Pass 2 wave B"`).
...
- **T-122** Canvas rebuild: Every page is a browser-chrome window with adjustable width and height and a live frame; windows drag (keyboard and pointer, never drag-only); layouts save to page_layouts and reload; a flows layer draws what each role can do as connectors between windows (from src/flows/roleFlows.ts); zoom 0.05..1; readable at 360 (list fallback) and 3840. _Deliverables_: src/modules/showcase/CanvasPage.tsx, src/modules/showcase/canvasLayout.ts, src/modules/showcase/flows.ts, docs/pages/D-21.md.
- **T-123** Homepage order by owner priority; role cards show only that role's surfaces: Order = what the firm owner cares about: staff work (front desk, attorneys, paralegals, owner) and the client app first, then site, board and tools, opposing-counsel portal near the bottom; every card lists only routes its role may open (D-048); previews still fill their frames (prompt 0005). _Deliverables_: src/modules/hub/HubPage.tsx, src/modules/hub/hub.css, docs/pages/HUB-01.md.
- **T-124** Demo simulator: Picking a route picks the device from its surface (customer -> phone, owner dashboards -> TV, other staff -> desktop, D-050) unless overridden; phone, tablet, laptop and TV chrome differ visibly; the tour steps use the right device; state stays in the query string. _Deliverables_: src/modules/showcase/SimulatorPage.tsx, src/components/organism/DeviceFrame/DeviceFrame.tsx, docs/pages/D-22.md.
- **T-125** Attorney document pipeline board (L-13) and order detail (L-14), paralegal queue (S-13): Kanban by stage with waiting-on badges (client / court / staff) and days waiting; order detail shows stage history, revision counter, client requests, DocPreview of the document and every action as a stage move through the domain function; paralegal queue lists what is on their desk by stage; keyboard move. _Deliverables_: src/modules/pipeline/BoardPage.tsx, src/modules/pipeline/OrderPage.tsx, src/modules/pipeline/QueuePage.tsx, docs/pages/L-13.md, docs/pages/L-14.md, docs/pages/S-13.md.
- **T-126** Client "my orders": Plain-language (en / es) stage names per order; a clear "we are waiting on you" block with the open requests; approve and request-changes actions write stage moves; no internal notes leak; phone first, 44 px targets. _Deliverables_: src/modules/client/OrdersPage.tsx, src/modules/client/OrderPage.tsx, docs/pages/C-11.md.
- **T-127** Front desk order status lookup: Typing a name, phone or order id shows the order's stage, who has it, what it waits on and the next date in words the desk can read to a caller; links to the call console; no legal memory or developer pages in the desk menu (D-048). _Deliverables_: src/modules/frontdesk/StatusPage.tsx, docs/pages/F-14.md.
- **T-128** Front desk call console (F-12: An incoming call (mock) matches a client and lists their orders with stage and waiting-on; the desk logs notes and an outcome as calls rows; follow-ups list every due date and item needed from a client with a done action; the directory links people to their orders; 10-foot legible on the wall screen. _Deliverables_: src/modules/frontdesk/CallsPage.tsx, src/modules/frontdesk/FollowUpsPage.tsx, src/modules/frontdesk/ClientsPage.tsx, docs/pages/F-12.md, docs/pages/F-13.md, docs/pages/F-15.md.
- **T-129** Drafting studio: Editing a document shows the pleading page with numbered lines and, beside it, the laws (from docs/legal rows, unverified flagged), precedent notes, recommendations and the client's details for the board node; "ask the client" creates client_requests the client sees in C-11 / C-21; templates browse by board node; every draft is a documents row with revision. _Deliverables_: src/modules/drafting/StudioPage.tsx, src/modules/drafting/TemplatesPage.tsx, src/data/schema/drafting.ts, src/data/seed/drafting.ts, docs/pages/S-21.md, docs/pages/S-10.md.
- **T-130** LMS: Courses group the 36 videos and the articles; the in-app player (YouTube embed, keyboard and remote friendly) writes lesson_progress; the journey picks what to watch next by board position; attorneys see what a client watched; admin reorders courses and lessons; Play is no longer a Placeholder. _Deliverables_: src/modules/learning/CoursesPage.tsx, src/modules/learning/PlayerPage.tsx, src/modules/learning/JourneyPage.tsx, src/modules/learning/InsightPage.tsx, src/modules/learning/AdminPage.tsx, docs/pages/C-40.md, docs/pages/C-41.md, docs/pages/L-40.md, docs/pages/A-11.md.
- **T-131** Client binder and evidence: Binder by board stage with DocPreview per item; upload (camera on phone) creates evidence rows with source, hash and dates; requests show what is missing and satisfy the pipeline's waiting-on-client state; connect page imports a mock email / text thread through the evidence seam; staff see the same binder. _Deliverables_: src/modules/binder/BinderPage.tsx, src/modules/binder/RequestsPage.tsx, src/modules/binder/ConnectPage.tsx, src/modules/binder/StaffBinderPage.tsx, src/data/schema/evidence.ts, docs/pages/C-20.md, docs/pages/C-21.md, docs/pages/C-22.md, docs/pages/L-31.md.
- **T-132** Game board verification against the poster PDF: Every square label and arrow checked against reference/game-board.pdf with a written diff; reconstructed edges listed; case mode uses the unified case ids; keyboard moves between connected squares; no console errors at any width. _Deliverables_: docs/game-board/nodes.json, docs/game-board/README.md, src/modules/board/, docs/pages/GB-01.md, docs/pages/GB-02.md, docs/pages/GB-03.md.
- **T-133** Attorney portraits and remaining site images: Every attorney portrait and any image the 2026-09-18 scrape missed is in reference/site-scrape/assets with a docs/data row (evidence scraped-live, verified false); people rows carry the names as shown on caltenantlaw.com badged unverified (D-046). _Deliverables_: docs/data/people.json, docs/data/illustrations.json, reference/site-scrape/assets/, src/data/seed/people.ts.
- **T-134** Pass 2 wave A integration, deep spacing and formatting pass across shells, hub and pages: Build green; _pending changelogs merged; no duplicate routes or codes; one spacing rhythm (4-pt tokens) across shells, hub and every new page; every new page has a spec at 100 % and a page doc; surfaces.md current. _Deliverables_: docs/changelog/0016-release-0.2.0.md, docs/data-model.md, docs/specs.md, src/styles/global.css, src/components/template/DesktopShell/DesktopShell.css.
- **T-135** Pass 2 wave A screenshots, responsive matrix, Spanish fill: Every new page captured at 390 + 1280 (dark + 3840 for key pages); qa:responsive 0 failing at the seven widths; client surfaces 100 % es, staff >= 80 %. _Deliverables_: docs/screenshots/**, docs/qa/responsive-report.md, src/modules/*/strings.ts.
- **T-136** Release 0.2.1: Deployed; release note with counts and open questions; takes over T-088's old dependencies (T-086, T-087). _Deliverables_: docs/changelog/00nn-release-0.2.1.md, package.json.

Wave A as a whole is done when: front desk, paralegal, attorney, marketing, client and opposing counsel each see only their surfaces (D-24 agrees with the menus); every order shows its stage and what it waits on to attorney, paralegal, client and front desk alike; every listed document, evidence item and lesson has a preview; the canvas is a free-form canvas of resizable browser windows with saved layouts and a flows layer; the simulator opens each demo in its device; the homepage reads in the owner's priority order; the LMS plays in-app; the board matches the poster; portraits are catalogued and badged; the formatting pass has run at the seven widths; release 0.2.0 is live with changelog 0016 and prompt 0006's `## Response` filled in.
...
Plan de construcción por pasadas: 0 descubrimiento (hecho), 1 fundación + primeros módulos en paralelo + versión 0.1.0, 2 operaciones centrales (casos, plazos, asignaciones, recepción, plantillas, carpeta del cliente y discovery, costos, aprendizaje, tienda, CRM, portal de la parte contraria), 3 comunicaciones y documentos (video, teléfono, correo, SMS, WhatsApp, firma, redacción, investigación, edición simultánea), 4 plataforma (Supabase, Stripe, tiempo real, WebMCP, voz, navegación con mando), 5 appliance y pulido (imagen Linux, 4K, manual completo, QA, propuesta). Las unidades son "ticks" de dependencia, no días. 136 tareas con ids permanentes (113 en la versión 0.1.0; la pasada 2 se dividió el 2026-09-20 en ola A, versión 0.2.0, y ola B, versión 0.2.1); cada una indica su modelo.
