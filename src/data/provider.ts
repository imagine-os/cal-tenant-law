...
/**
 * The data contract every page talks to. MockProvider today (localStorage), SupabaseProvider later (Postgres + RLS +
 * realtime channels) - same interface, so pages never know which one is behind useData(). Writes go by id; lists
 * re-render from `subscribe` (P-14).
 */
