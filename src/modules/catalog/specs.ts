...
const NOTE = 'Catalog data is docs/data/services-catalog.json, reconstructed from indexed search results because the live site is unreachable from the build environment. Every price is "as listed on the current site" with an unknown crawl date and renders unverified until an attorney confirms it (D-025, RULE-CATALOG-01).';
...
export const servicesSpec = defineSpec({
  ...
  logic: [
    'Categories render in the store’s own order (service_categories.sort_order); a category with no active service in the current filter is hidden.',
...
export const serviceDetailSpec = defineSpec({
  ...
  logic: [
    ...
    'The sources block lists the store URLs the facts were indexed from and whether each was quoted or reconstructed (D-025).',
...
export const outlineSpec = defineSpec({
  ...
  purpose: 'The whole catalog as one collapsible outline \u2014 category, sub-category, service \u2014 with the SKU, the price, the time expectation, what the client has to provide and what they receive on every row. The view to read top to bottom, expand one branch at a time, or print and mark up.',
  layout: ['SiteLayout', 'Hero (what the outline is, counts)', 'Tools (expand all, collapse all, print)', 'OutlineTree (category > sub-category > service)', 'PrintNote'],
  data: ['service_categories', 'services'], roles: EVERYONE,
  logic: [
    ...
    'Categories nest through service_categories.parent_id; three grouping parents (talk to an attorney, free and do-it-yourself, discovery) are a CTL OS grouping of the store\u2019s flat categories and say so in their description.',
    'Root categories are open on first load so the page reads as a table of contents; expand-all and collapse-all switch the whole tree.',
  ...
  states: ['roots open (default)', 'fully expanded', 'fully collapsed', 'a row focused by keyboard', 'print', 'Spanish', 'dark', '10-foot (>= 2560)'],
  notes: [NOTE, PULLED, 'time_expectation, client_inputs and deliverable_format are null on the services the current site does not describe; those rows say "to be confirmed" and the live-scrape pass fills them.'],
  checkedAt: CHECKED,
});
