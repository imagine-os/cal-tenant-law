...
CTL OS is the operations system for California Tenant Law (caltenantlaw.com): public website, tenant app, front desk, attorney and paralegal workspaces with the document pipeline and the drafting studio, the client's evidence binder, the learning system, owner and admin dashboards, the eviction game board, ops manual, docs and dev tools in one codebase. **Release 0.2.0** (2026-09-20) is live at https://imagine-os.github.io/cal-tenant-law/.

**Version 0.2.0** (Pass 2 wave A, 2026-09-20): Justin's fifteen asks of prompt 0006 in one release - a formatting pass across every shell and page; roles see only what is theirs (D-048, role matrix at `/#/dev/roles`); the hub ordered by what the owner cares about with per-role page lists and live counts; the demo simulator on real device chrome; the canvas rebuilt as a zoomable world of resizable browser windows with role-flow lines; document previews from metadata everywhere; the attorney document pipeline with "waiting on the client" on every surface (L-13 / L-14 / S-13), the client's own orders view (C-11) and the front desk's status lookup (F-14); the front desk call console, follow-ups and client directory (F-12 / F-15 / F-13); the drafting studio on pleading paper with laws, precedents, recommendations and client details beside the sheet (S-21 / S-22 / S-10); the evidence binder with uploads, requests and channel imports (C-20 / C-21 / C-22 / L-31); the LMS from the firm's videos and articles with a click-to-play player (C-40 / C-41 / C-42 / L-40 / A-11); the attorneys page and public video library (P-05 / P-06); the game board verified against the poster; and the hub's Visual explorations links (prompt 0007). 81 routes, 80 built, 45 tables, 69 components, 426 action ids. Release notes: `docs/changelog/0029-release-0.2.0.md`.
